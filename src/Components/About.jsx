...
//       relative   min-h-screen pt-20 transition-colors duration-300"
...
const container = {
  ...
  visible: {
    transition: {
      ...
      staggerChildren: 0.1, // 0.25 ki jagah
...
function SkillCard({ skills }) {
  ...
  return (
    <motion.div
      ...
      whileInView="visible"
      viewport={{ once: false }}
      className="xl:flex xl:flex-wrap gap-2 leading-relaxed lg:justify-start grid justify-center sm:grid-cols-2 place-items-center "
    >
      {skills.map((skill, index) => (
        <motion.div key={index} variants={item}>
          <div
            className={`relative inline-flex items-center gap-2 lg:px-3 rounded-full xl:py-2 transition-transform duration-300 tracking-widest md:text-[14px] font-semibold text-[12px] px-[6px] py-[2px] justify-center cursor-text
              ${
                hoverIndex !== null
                  ? hoverIndex === index
                    ? "bg-gray-700/80 scale-105 shadow-lg shadow-blue-500/20"
            ...
            onMouseEnter={() => setHoverIndex(index)}
            onMouseLeave={() => setActiveIndex(index) || setHoverIndex(null)}
          >
            <span>{skill.icon}</span>
...
export function About() {
  ...
  return (
    <div
      id="about"
      className="py-20 px-4 xl:px-[0px] flex items-center relative min-h-screen pt-20 transition-colors duration-300 
   "
    >
      <div className="grid lg:flex justify-center gap-16 items-center     overflow-hidden">
        {/* About Text */}
        <motion.div
          className="about-text rounded-3xl shadow-2xl space-y-6"
          variants={container}
        ...
        >
          <motion.h2
            variants={item}
            className="text-xl md:text-2xl xl:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-500 text-center xl:text-left"
          >
            About me
          </motion.h2>
          ...
          <motion.h3
            variants={item}
            className="text-xl xl:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-500 text-center xl:text-left md:text-2xl"
          >
            Here are my main skills:
          </motion.h3>

          <SkillCard skills={skillCategories} />
        </motion.div>
